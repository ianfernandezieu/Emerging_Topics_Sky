"""Generate a text-only version of the academic report (no figures, no tables).

Same structure, headings, and prose as report.docx but without any embedded
images, tables, or figure captions.
"""
...
OUT_PATH = ROOT / "report_text.docx"
...
def main():
    ...
    doc.add_paragraph(
        "Key statistics: 3,346 daily records for Madrid-Barajas (LEMD), date range "
        "2017-01-01 to 2026-02-28, average 976 movements per day, minimum 2 (during "
        "COVID lockdown), maximum 1,600. Zero missing dates across the full range."
    )
    ...
    doc.add_paragraph(
        "Flight volume (4 features): arrivals, departures, total_movements, "
        "arr_dep_imbalance. These are direct congestion indicators.",
        style="List Bullet"
    )
    doc.add_paragraph(
        "Calendar (15 features): dow, month, is_weekend, is_holiday, is_bridge_day, "
        "and cyclic encodings (sin/cos for day-of-week, month, day-of-year). These capture "
        "demand patterns at weekly, seasonal, and holiday scales.",
        style="List Bullet"
    )
    doc.add_paragraph(
        "Weather (11 features): temperature, precipitation, wind_speed, wind direction "
        "(sin/cos), surface pressure, humidity, cloud cover, is_severe_weather, is_raining. "
        "These capture operational disruption signals.",
        style="List Bullet"
    )
    doc.add_paragraph(
        "Lag features (14 features): acps_lag_1d/2d/3d/7d/14d/28d/365d and matching "
        "movements lags. These capture temporal autocorrelation and trends.",
        style="List Bullet"
    )
    doc.add_paragraph(
        "Rolling windows (9 features): acps_rmean_7d/14d/28d, movements_rmean_*, "
        "acps_rstd_28d, movements_yoy_change. These provide smoothed trend and "
    ...
    doc.add_paragraph(
        ...
        "max_iter=200 (sufficient boosting rounds for 2,322 training samples), "
        "max_depth=6 (allows moderate interaction complexity without overfitting), "
        "learning_rate=0.1 (standard rate; lower values showed no improvement), "
    ...
    doc.add_paragraph(
        "The ACPS distribution is bimodal: a cluster of low scores (0-35) corresponds "
        "to the COVID period, while the main mass (60-78) represents normal operations. "
        "The three congestion classes split at the 60th and 85th percentiles, yielding "
        "1,980 Low days (59.7%), 837 Medium days (25.2%), and 501 High days (15.1%)."
    )

    doc.add_heading("5.2 Regression Performance", level=2)

    doc.add_paragraph(
        "The HistGradientBoosting regressor was evaluated across all three data splits. "
        "On the training set (2,322 days): MAE = 0.15, RMSE = 0.31, R-squared = 0.9998. "
        "On the validation set (498 days): MAE = 0.33, RMSE = 0.49, R-squared = 0.9858. "
        "On the test set (498 days): MAE = 0.45, RMSE = 0.69, R-squared = 0.9613."
    )
    doc.add_paragraph(
    ...
    doc.add_paragraph(
        "The residual analysis reveals no systematic patterns: residuals "
    ...
    doc.add_paragraph(
        "The classifier achieves 90.2% overall accuracy on the test set with balanced "
        "performance across all three classes. Per-class results: High congestion -- "
        "precision 0.93, recall 0.88, F1 0.90 (184 support); Low congestion -- precision "
        "0.85, recall 0.97, F1 0.91 (70 support); Medium congestion -- precision 0.90, "
        "recall 0.90, F1 0.90 (244 support). Weighted average F1 is 0.90. Balanced "
        "accuracy is 0.92."
    )
    doc.add_paragraph(
        "Two observations are particularly noteworthy. First, the model has very high "
        "recall for Low congestion (0.97), meaning it almost never misclassifies a quiet "
        "day as busy. This is operationally valuable: it prevents unnecessary resource "
        "deployment on genuinely calm days. Second, the model has high precision for High "
        "congestion (0.93), meaning when it predicts a high-congestion day, it is correct "
        "93% of the time. This reliability is critical for actionable alerts -- false "
        "alarms would erode trust in the forecasting system."
    )
    doc.add_paragraph(
    ...
    doc.add_paragraph(
        ...
        "direct correlations found in EDA. Temperature shows the strongest relationship "
        "(r = 0.11), likely as a proxy for season. However, their collective contribution "
        "helps explain outlier days: severe weather events cause cancellations and delays "
        "that pure traffic-volume features cannot predict."
    )
    ...
    for member, role, contrib in contribs:
        p = doc.add_paragraph()
        run = p.add_run(f"{member} -- {role}: ")
        run.bold = True
        p.add_run(contrib)

    # Save
    doc.save(str(OUT_PATH))
    print(f"Text-only report saved to {OUT_PATH}")
